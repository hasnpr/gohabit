...
func TestSupervisor_Register(t *testing.T) {
	...
	t.Run("successful registration", func(t *testing.T) {
		...
		s.Register("register-test-process", handler)
		...
		if s.IsRunning("register-test-process") {
	...
	t.Run("duplicate name registration should panic", func(t *testing.T) {
		...
		defer func() {
			if r := recover(); r == nil {
				...
				expectedMsg := `process name "register-test-process" already in use`
				if r != expectedMsg {
		...
		s.Register("register-test-process", handler) // Same name as above - should panic
	})
...
func TestSupervisor_RestartPolicies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			processName := fmt.Sprintf("restart-test-%d", time.Now().UnixNano())
			s.Register(processName, handler,
...
func TestSupervisor_ConcurrentOperations(t *testing.T) {
	...
	for i := 0; i < numProcesses; i++ {
		...
		go func(id int) {
			...
			s.Register(fmt.Sprintf("concurrent-%d-%d", testID, id), handler)
		}(i)
	}
	...
	for i := 0; i < numProcesses; i++ {
		...
		go func(id int) {
			...
			name := fmt.Sprintf("concurrent-%d-%d", testID, id)
...
func TestSupervisor_ConcurrentShutdown(t *testing.T) {
	...
	s.Register("concurrent-shutdown-test", handler)
	s.Run()
...
func TestSupervisor_WithRestartOptions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			processName := fmt.Sprintf("restart-options-test-%d", time.Now().UnixNano())
			s.Register(processName, handler,
				WithRestart(tt.policy, tt.maxRestarts, tt.delay))

			// Process is registered but not running until Run() is called
			if s.IsRunning(processName) {
				t.Error("Process should not be running before Run() is called")
			}
...
func TestSupervisor_IsRunning(t *testing.T) {
	...
	s.Register("isrunning-test-process", handler)

	// Process should not be running after registration
	if s.IsRunning("isrunning-test-process") {
		t.Error("Process should not be running before Run() is called")
	}
	...
	// Wait for process to start
	waitForStatus(t, s, "isrunning-test-process", StatusRunning, time.Second)
...
func TestSupervisor_NoOpMetrics(t *testing.T) {
	s := createTestSupervisor(2 * time.Second)

	// Don't enable metrics - should use NoOp metrics by default
	var execCount atomic.Int32
	handler := func(ctx context.Context) error {
		count := execCount.Add(1)
		if count == 1 {
			return errors.New("first execution fails to trigger restart")
		}
		<-ctx.Done()
		return ctx.Err()
	}

	// Register with restart policy to trigger various metrics calls
	s.Register("noop-metrics-test", handler,
		WithRestart(RestartOnFailure, 2, 10*time.Millisecond))
	s.Run()

	// Wait for initial execution, restart, and second execution
	time.Sleep(100 * time.Millisecond)

	// Verify process is running (after restart)
	if !s.IsRunning("noop-metrics-test") {
		t.Error("Process should be running after restart")
	}

	// Shutdown (this should not panic even with NoOp metrics)
	s.Shutdown()

	if execCount.Load() < 2 {
		t.Error("Process should have executed at least twice (initial + restart)")
	}
}
...
func TestSupervisor_FullLifecycle(t *testing.T) {
	...
	expectedEvents := []string{"execute-1", "execute-2", "shutdown-received"}
